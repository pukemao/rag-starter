...
本项目提供一组基于 LangChain 的本地文档加载器封装，代码位于 `src/loader`。加载器按文档类型拆分为独立 Python 模块，调用方可以通过统一注册表按文件扩展名自动选择加载器。
...
更多说明见 [src/loader/README.md](src/loader/README.md)。

## 测试

测试使用 Python 标准库 `unittest`，不依赖真实 LangChain 安装即可验证注册表和懒导入逻辑：
